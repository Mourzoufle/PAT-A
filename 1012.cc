...
int main() {
	...
	map<int, int> indices;														// map of student ids and their entries in the matrix of scores - for query
	for (int i = 0; i < num_student; i++) {
		int id;
	...
	for (int i = 0; i < num_query; i++) {
		int id;
		scanf("%d", &id);
		if (indices.find(id) == indices.end()) {								// ID not found
			printf("N/A\n");
			continue;
		}
		int idx_student = indices[id], min_rank = num_student + 1, idx_course;	// ensure the initial value of min rank is out of range
		for (int j = 0; j < 4; j++) {
