...
struct Player {
	...
	Player(int time_arrive, int time_play): time_arrive(time_arrive), time_play(min(time_play * 60, 7200)) {};				// no player can play more than 2 hour (7200 seconds)
};
...
int main() {
	...
	while (!commons.empty() || !vips.empty()) {
		...
		int time_serve = max(tables[cur_table].time_serve, time_arrive);													// game begins when both players and the table are ready 
