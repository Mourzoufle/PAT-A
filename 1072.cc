...
int main() {
	...
	int location;
	double max_min = 0, min_avg = DBL_MAX;
	for (int i = num_house; i < num_node; i++) {
		...
		if ((cur_min > max_min) || ((cur_min == max_min) && (cur_avg < min_avg))) {	// found a better location
			location = i;
			max_min = cur_min;
			min_avg = cur_avg;
		}
	...
	if (min_avg == DBL_MAX)															// no solution has ever found
		printf("No Solution");
	else
		printf("G%d\n%.1lf %.1lf", ++location - num_house, max_min, min_avg / num_house);
